...
switch (input.fPort) {
  ...
  if(mode!=6)
  {
    // Jeweilige Zeile auskommentieren ANFANG
    
    //DO = 1: true (Ausgang ist aktiv, Low) L
    //DO = 0: false (Ausgang ist inaktiv, High H 
    
    //data.DO1_status= (input.bytes[8] &0x01)? "L":"H";
    //data.DO1_status= (input.bytes[8] &0x01)? 1:0;
    data.DO1_status= (input.bytes[8] &0x01)? true:false;
    
    //data.DO2_status= (input.bytes[8] &0x02)? "L":"H";
    //data.DO2_status= (input.bytes[8] &0x02)? 1:0;
    data.DO2_status= (input.bytes[8] &0x02)? true:false;
    
    
    data.RO1_status= (input.bytes[8] &0x80)? true:false;
    //data.RO1_status= (input.bytes[8] &0x80)? "ON":"OFF";
    //data.RO1_status= (input.bytes[8] &0x80)? 1:0;
    
    data.RO2_status= (input.bytes[8] &0x40)? true:false;
    //data.RO2_status= (input.bytes[8] &0x40)? "ON":"OFF";
    //data.RO2_status= (input.bytes[8] &0x40)? 1:0;
    
    // Jeweilige Zeile auskommentieren ENDE
