function decodeUplink(input) {
    ...
    function handleKeepalive(bytes, data){
        ...
        const min = 2.1;
        const div = max - min;
        const actDiv = Math.min(Math.max(data.batteryVoltage, min), max) - min;
        ...
        commands.map(function (command, i) {
            switch (command) {
                ...
                case '58':
                    {
                        ...
                        var temperatureRestoredAfterManualBoost = !!(notificationByte & 0x01);
                        var temperatureChangedByHeatingSchedule = !!(notificationByte & 0x02);
                ...
                case '5a':
                    {
                        command_len = 41;
                        var eventsGroup = parseInt(commands[i + 1], 16);
                        ...
                        for (var eventIdx = 0; eventIdx < eventsToProcess; eventIdx++) {
                            ...
                            var weekdays = {
                                monday: !!(weekdayByte & 0x01),
                                tuesday: !!(weekdayByte & 0x02),
                                wednesday: !!(weekdayByte & 0x04),
                                thursday: !!(weekdayByte & 0x08),
                                friday: !!(weekdayByte & 0x10),
                                saturday: !!(weekdayByte & 0x20),
                                sunday: !!(weekdayByte & 0x40)
                            };
                ...
                case '64':
                    {
                        ...
                        var durationInSeconds = ledDurationValue / 2;
                ...
                case '6c':
                    {
                        ...
                        var eventsHighByte = parseInt(commands[i + 2], 16);
                        var eventsMidByte = parseInt(commands[i + 3], 16);
                        var eventsLowByte = parseInt(commands[i + 4], 16);
